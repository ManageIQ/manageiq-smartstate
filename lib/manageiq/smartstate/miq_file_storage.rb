    ...
    class FileStorage
      class InvalidSchemeError < ArgumentError
        ...
        def error_message(bad_scheme)
          valid_schemes = ::ManageIQ::Smartstate::FileStorage.storage_interface_classes.keys.inspect
          "#{bad_scheme} is not a valid FileStorage uri scheme. Accepted schemes are #{valid_schemes}"
        end
      end
      ...
      class Interface
        ...
        #
        # Classes that inherit from `FileStorage` need to make sure to create a
        ...
        # should be done here (see `GenericMountSession#upload_single` for an
        # example)
        #
        ...
        # `dest_uri` is a file path (in `GenericMountSession#upload_single`, this
        # is converted to `relpath`), this does not need to be converted to a
