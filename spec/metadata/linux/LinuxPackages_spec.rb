  ...
  context "with an RPM directory" do
    ...
    context "with a rpmdb.sqlite DB file" do
      before do
        ...
        expect(fs).to receive(:fileSize).with(File.join(MiqLinux::Packages::RPM_DB, "rpmdb.sqlite")).and_return(File.size(File.expand_path('data/rpm/rpmdb.sqlite', __dir__)))
        expect(fs)
          .to receive(:fileOpen)
